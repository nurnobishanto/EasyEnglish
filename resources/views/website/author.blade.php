                ...
                <div class="col-lg-8 col-md-12">
                    <div class="row justify-content-center">
                        ...
                        @else
                                ...
                                <div class="col-lg-6 col-md-6">
                                    <div class="single-blog-card">
                                        <div class="blog-image">
                                            ...
                                            @else
                                                <a href="{{ Route('website.post', ['slug' => $post->slug]) }}"><img
                                                        src="{{ asset('website') }}/assets/images/blog/blog-1.jpg"
                                                        alt="{{ $post->title }}"></a>
                                            @endif
                                        ...
                                        <div class="blog-content with-padding">
                                            @if ($post->user)
                                                <span>By <a
                                                        href="{{ Route('website.author', ['slug' => $post->user->id]) }}">{{  ($post->user->name) }}</a></span>
                                            @else
                                            @endif
                                            ...
                                            <ul class="entry-meta">
                                                <li><i class="ri-calendar-line"></i>
                                                    {{  ($post->created_at->format('M d, Y')) }}</li>
                                                @if ($post->category)
                            ...
                            @endforeach
                            <span>{{ $authorposts->count() }} {{  ('Posts Found') }}</span>
                        @endif
                    </div>

                    {{ $authorposts->links('vendor.pagination.custom') }}
